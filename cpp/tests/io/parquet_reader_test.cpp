...
TEST_F(ParquetReaderTest, NullableStructBoolAndStringUserBounds)
{
  ...
  std::vector<bool> parent_validity(row_count);
  std::vector<bool> bool_validity(row_count);
  std::vector<bool> string_validity(row_count);
  for (int row = 0; row < row_count; ++row) {
    strings.push_back("nested-" + std::to_string(row % 17));
    parent_validity[row] = row % 11 != 0;
    bool_validity[row]   = row % 7 != 0;
    string_validity[row] = row % 5 != 0;
  }
  auto bool_values =
  ...
  cudf::test::fixed_width_column_wrapper<bool> bool_child(
    bool_values, bool_values + row_count, bool_validity.begin());
  cudf::test::strings_column_wrapper string_child(
    strings.begin(), strings.end(), string_validity.begin());
  auto struct_col =
    cudf::test::structs_column_wrapper{{bool_child, string_child}, parent_validity.begin()};
  cudf::table_view const expected{{struct_col}};
...
TEST_F(ParquetReaderTest, NullableStructDictionaryAndByteStreamSplit)
{
  constexpr int row_count = 257;
  std::vector<bool> parent_validity(row_count);
  std::vector<bool> int_validity(row_count);
  std::vector<bool> float_validity(row_count);
  std::vector<bool> string_validity(row_count);
  std::vector<std::string> strings;
  strings.reserve(row_count);
  for (int row = 0; row < row_count; ++row) {
    parent_validity[row] = row % 11 != 0;
    int_validity[row]    = row % 7 != 0;
    float_validity[row]  = row % 5 != 0;
    string_validity[row] = row % 3 != 0;
  ...
  cudf::test::fixed_width_column_wrapper<int> int_child(
    int_values, int_values + row_count, int_validity.begin());
  cudf::test::fixed_width_column_wrapper<float> float_child(
    float_values, float_values + row_count, float_validity.begin());
  cudf::test::strings_column_wrapper string_child(
    strings.begin(), strings.end(), string_validity.begin());
  auto struct_col = cudf::test::structs_column_wrapper{{int_child, float_child, string_child},
                                                       parent_validity.begin()};
  cudf::table_view const expected{{struct_col}};
...
// Exercise BYTE_STREAM_SPLIT under both nesting and repetition. The nullable STRUCT and LIST levels
// add definition and repetition streams; 32-row pages and the selected ranges cover both page
// boundaries and the 32-value decode boundary.
TEST_F(ParquetReaderTest, ByteStreamSplitNestedListFloat)
{
  ...
  for (cudf::size_type row = 0; row < num_rows; ++row) {
    ...
    list_validity.push_back(list_is_valid);
    struct_validity.push_back(struct_is_valid);
